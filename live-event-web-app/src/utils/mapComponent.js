  ...
  return (
    <>
      {state.markers.map((marker, index) => ( marker.activ ? (
        <AdvancedMarker className="advanced-marker" key={index} position={{ lat: marker.lat, lng: marker.lng }} onClick={() => setSelectedMarker(marker)}>
          <span role="img" aria-label="dog-marker"> {marker.pinIcon} </span>
        </AdvancedMarker>
        ) : null ))}
 {selectedMarker && (
    <InfoWindow className='info-window' position={{ lat: selectedMarker.lat, lng: selectedMarker.lng }} onCloseClick={() => setSelectedMarker(null)} >
